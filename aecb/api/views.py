    ...
    def get(self, request, format=None):
        try:
            ...
            if auth_result.account is None:
                ...
                if not instance:
                    raise Exception("Unregistered user {email}".format(email=auth_result.email_from_token))

                new_account = Account(role=role, token=token, user=instance)
                new_account.save()

                return HttpResponse(json.dumps({"detail": "Account created for {email}".format(email=auth_result.email_from_token)}), status=status.HTTP_200_OK)

            else:
                auth_result.account.token = token
                auth_result.account.save()

                return HttpResponse(json.dumps({"detail": "Auth as {role}".format(role=auth_result.account.role)}), status=status.HTTP_200_OK)

        except Exception as e:
            return HttpResponse(json.dumps({"detail": str(e)}), status=status.HTTP_400_BAD_REQUEST)
...
class ClientViewSet(viewsets.ModelViewSet):
    ...
    def available_credit_cards(self, request, pk=None):
        ...
        response = requests.get("{score_microservice}/score".format(
            score_microservice=settings.AECB_EXTERNAL_API), params=client_params)

        if not response.ok:
            return HttpResponse("Error: {endpoint} sent code {status} in response.".format(endpoint=settings.AECB_EXTERNAL_API, status_code=response.status_code), status=status.HTTP_503_SERVICE_UNAVAILABLE)
    ...
    def request_credit_card(self, request, pk=None):
        try:
            ...
            if active_preapproval is None:
                ...
                raise Exception("There is an active preapproval request issued to {email}".format(email=pk))
...
class PreapprovalRequestViewSet(viewsets.ModelViewSet):
    ...
    def generate_report(self, request):
        ...
        active_ratio = (preapproval_stats.get("total_active_cards") / self.queryset.count()) * 100
        approval_ratio = (preapproval_stats.get("total_approved_cards") / self.queryset.count()) * 100
    ...
    def generate_pdf(self, report_pk):
        ...
        env = Environment(loader=FileSystemLoader(os.path.join(settings.BASE_DIR, "aecb", "templates")))
        template = env.get_template("report.html")
        html_string = template.render(report=serializer.data)

        filename = "{pk}_report_{random}.pdf".format(pk=report_pk, random="".join(random.choices(string.ascii_uppercase + string.digits, k=10)))
        ...
        return filename if report_created else None
